...
def main():
    ...
    for word in words:
        data = {
            "requestType": "session",
            "trackId": "8430199d-98da-4416-8148-62c980825b8a",
            "priceUplift": 0,
            "keyword": word
        }
        ...
        for item in res_json["data"].get("wordList", []):
            ...
            datas.append(data)
    current_date_str = datetime.date.today().strftime('%Y-%m-%d')
    with open("./data/jgData/" + current_date_str + '_shangxiayou_data.json', 'w', encoding='utf-8') as file:
        json.dump(datas, file, ensure_ascii=False, indent=4)
        print(f"保存{"./data/jgData/" + current_date_str + '_shangxiayou_data.json'}数据成功")
